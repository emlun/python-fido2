...
def miller_eval_point(P: PointAffine, Q: PointAffine) -> Tuple[Polynomial, MillerLog5]:
    ...
    for i in reversed(range(n - 2 + 1)):
        ...
        if (r >> i) % 2 == 1:
            RP = R + P
            if R == -P:
                l = fy.promote(vertical_fn(P, fy))
                v = fy.promote(1)
            else:
                l = intersect_fn(R, P, fy)
                v = fy.promote(vertical_fn(RP, fy))
            R = RP
            updn = l.eval(Q)
            updd = v.eval(Q)
            upd = updn / updd
...
type MillerLog3 = list[
    Tuple[Polynomial, None, Polynomial] | Tuple[Polynomial, Polynomial, Polynomial]
]
